...
namespace WBrowser
{
    public partial class net_shikhar : Form
    {
        ...
        private void addFavorit(String url, string name)
        {
            ...
            else
            {
                ...
                MessageBox.Show("favourities not yet done.");
            }
                ...
                if (favoritesPanel.Visible == true)
                {
                    ...
                    if (comboBox1.Text.Equals("Ordered Visited Today"))
                    {
                        if (!historyTreeView.Nodes.ContainsKey(url.ToString()))
                        {
                            ...
                            node.ToolTipText = url.ToString() + "\nअंतिम बार देखा गया: " + data + "\nकितने समय बार देखा गया: :" + i.ToString();
                    ...
                    if (comboBox1.Text.Equals("साइट के द्वारा देखें"))
                    {
                    ...
                    if (comboBox1.Text.Equals("दिनांक के आधार पर देखें"))
                    {
...
//visible change
        ...
        {
            net_shikhar new_form = new net_shikhar();
            new_form.Show();
...
//show history in tree wiew
        private void showHistory()
        {
            ...
            if (File.Exists(historyXml))
            {
                ...
                if (comboBox1.Text.Equals("Ordered Visited Today"))
                {
                    ...
                    foreach (XmlElement el in myXml.DocumentElement.ChildNodes)
                    {
                        ...
                        node.ToolTipText = el.GetAttribute("url") + "\nअंतिम बार देखा गया: " + el.GetAttribute("lastVisited") + "\nकितने समय बार देखा गया:" + el.GetAttribute("times");
                        node.Name = el.GetAttribute("url");
                        node.ContextMenuStrip = histContextMenu;
           ...
           {  
               historyTreeView.ShowRootLines = true;
               historyTreeView.Nodes.Add("2 सप्ताह पहले", "2 सप्ताह पहले", 2, 2);
               historyTreeView.Nodes.Add("पिछले हफ्ते", "पिछले हफ्ते", 2, 2);
               historyTreeView.Nodes.Add("इस सप्ताह", "इस सप्ताह", 2, 2);
               historyTreeView.Nodes.Add("कल", "कल", 2, 2);
               historyTreeView.Nodes.Add("आज", "आज", 2, 2);
               foreach (XmlElement el in myXml.DocumentElement.ChildNodes)
               {
                  DateTime d=DateTime.Parse(el.GetAttribute("lastVisited"),currentCulture);
...
//calculator
       ...
       private void helpToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            //
            MessageBox.Show("nothing in help");
        }
 #endregion         
